...
Deno.serve(async (req) => {
  ...
  if (convo.channel_type === "instagram") {
    ...
    // WhatsApp/Messenger/SMS/LINE/Kakao not yet wired — mark sent locally (no
    // provider) so the app still works for those channels in dev; real sends are
    // per-channel chunks behind their own *_LIVE flags.
    sendResult = { ok: true };
  }
